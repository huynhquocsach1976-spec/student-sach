...
def get_gemini_client(key):
    if not key:
        return None
    # Khởi tạo client chính thức của thư viện google-genai
    return genai.Client(api_key=key)

client = get_gemini_client(api_key)

# Hàm gọi Gemini sinh email can thiệp
def generate_intervention_email(student_info):
    if not client:
        return "⚠️ Chưa cấu hình GEMINI_API_KEY trong Streamlit Secrets."
    
    # Xây dựng Prompt cá nhân hóa dựa trên dữ liệu học viên
    prompt = f"""
    ...
    try:
        # Sử dụng mô hình gemini-2.5-flash tối ưu cho văn bản nhanh và chính xác
...
# -----------------------------------------------------------------------------
# 2. GIAO DIỆN TÍCH HỢP TRONG STREAMLIT
# -----------------------------------------------------------------------------
st.markdown("---")
st.header("🤖 AI Assistant: Tự Động Soạn Email Can Thiệp Cá Nhân Hóa")

# Giả lập dữ liệu danh sách nguy cơ nếu đã có df_at_risk từ các bước trước
# (Đoạn này kết nối trực tiếp với DataFrame học viên bị cảnh báo)
if 'df_at_risk' in locals() and len(df_at_risk) > 0:
    
    # Tạo danh sách chọn học viên
    ...
    st.info("Vui lòng tải file dữ liệu lên để xem danh sách học viên nguy cơ và trải nghiệm tính năng soạn email bằng AI.")